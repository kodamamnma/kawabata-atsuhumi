...
- **プライバシーポリシー：アクセス解析ツール条文を更新し、第三者配信広告（Google AdSense）に関する条文を新規追加**（`kawabata-wp-theme/page-privacy.php`、`privacy.html`）
  - `page-privacy.php`: 第4条（アクセス解析ツールについて）の文言をGoogleアナリティクス限定の表現に更新。新たに第5条（第三者配信の広告サービスについて／Google AdSense）を追加し、以降の条番号（旧第5条・第6条）を第6条・第7条に繰り下げ。
  - `privacy.html`（旧プロトタイプ）: 対応する「アクセス解析」項目を同文言に更新し、「第三者配信の広告サービスについて」項目を追加。ただし本ファイルは既に利用規約セクションが無いなど `page-privacy.php` と構成が乖離しており、完全な同一内容ではない点に留意。
